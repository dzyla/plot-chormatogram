...
def plot_chromatogram(
    ...
    if do_fractions:
        ...
        for n, (ml, fraction) in enumerate(
            zip(data[column_fractions], data["Fraction"])
        ):
            ml = float(ml)
...
if uploaded_file is not None:
    ...
    with st.expander("Plotting Parameters", expanded=True):
        ...
        if len(numeric_cols) > 0:
            ...
            ymin, ymax = st.slider("Y-axis Limits", int(data[numeric_cols].max().max())*-2, int(data[numeric_cols].max().max())*2, (0, int(data[numeric_cols].max().max())))
    ...
    for i in range(num_traces):
        with st.expander(f"Trace {i+1} Settings"):
            ...
            y_column = st.selectbox("Y column", numeric_cols, index=1 if len(numeric_cols) > 1 else 0, key=f"y_column_trace_{i}")

            color = st.color_picker("Color", value=st.session_state.random_colors[i], key=f"color_trace_{i}")
            line_width = st.slider("Line Width", 1, 5, 2, key=f"line_width_trace_{i}")
            plot_every = st.slider("Plot Every N Fractions", 1, 10, 1, key=f"plot_every_trace_{i}")
            sample_name = st.text_input("Sample Name", y_column, key=f"sample_name_trace_{i}")
            ...
            trace_params.append((x_column, y_column, color, plot_every, sample_name, mod_y, y_scale, line_width))
    

    # Plot button
    if st.button("Plot Chromatogram"):
        fig, ax = plt.subplots(figsize=(figsize_width, figsize_height), dpi=figsize_dpi)
        for x_column, y_column, color, plot_every, sample_name, mod_y, y_scale, line_width in trace_params:
            plot_chromatogram(ax, data, x_column, y_column, plot_every, title, sample_name, mod_y, y_scale, (x_lim_min, x_lim_max), ymin, ymax, if_fractions, "ml.5", 0, color, line_width)
        st.pyplot(fig)
